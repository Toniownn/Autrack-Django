...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:ring-[3px] focus-visible:ring-ring/50",
  {
    variants: {
      variant: {
        // ✅ Matches your Banner gradient
        default:
          "bg-gradient-to-r from-orange-600 to-orange-400 text-white hover:from-orange-500 hover:to-orange-300 shadow-md hover:shadow-lg transition-all duration-200",
      ...
      size: {
        default: "h-10 px-6 py-2 rounded-full has-[>svg]:px-4",
        sm: "h-8 rounded-md gap-1.5 px-3 has-[>svg]:px-2.5",
        lg: "h-12 rounded-full px-8 has-[>svg]:px-5 text-base font-semibold",
        icon: "size-9",
      },
    },
    defaultVariants: {
